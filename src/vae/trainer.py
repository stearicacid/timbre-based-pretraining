...
class Trainer:
    def __init__(
        ...
        Args:
            cfg: Hydra config
            logger: logger instance (optional, uses global if None)
        """
        self.cfg = cfg
    ...
    def train_epoch(
        ...
        for batch_idx, batch_data in enumerate(train_pbar):
            ...
            try:
                ...
                loss_weights = None
                if hasattr(self.cfg.model.loss, 'loss_weights'):
                    loss_weights = OmegaConf.to_container(self.cfg.model.loss.loss_weights, resolve=True)
                ...
                if (hasattr(self.cfg.optimization, 'gradient_clipping') and 
                    self.cfg.optimization.gradient_clipping.enabled):
